...
/**求某个窗口中前N名的热门点击商品，key为窗口的时间戳，输出为TopN的结果字符串*/
public class TopNHotItems extends KeyedProcessFunction<Long, ItemViewCount, String>{
    ...
    public void onTimer(long timestamp, OnTimerContext ctx, Collector<String> out) throws Exception {
        // 获取收到的所有商品点击量
        List<ItemViewCount> allTtems = new ArrayList<>();
        ...
        itemState.clear();
        // 按照点击量从大到小排序
        allTtems.sort(new Comparator<ItemViewCount>() {
            @Override
        ...
        for (int i=0; i<topSize; i++){
            ItemViewCount currentItem = allTtems.get(i);
